...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // general IO
    static int getInt() throws IOException { return nextInt(); }
    static long getLong() throws IOException { return nextLong(); }
    static double getDouble() throws IOException { return Double.parseDouble(next()); }
    ...
    // array IO
    static int[] arrint() throws IOException { String line = nextLineNonEmpty(); String[] str = line.trim().split("\s+"); int n = str.length;  int[] x = new int[n]; for (int i = 0; i < n; i++) x[i] = Integer.parseInt(str[i]); return x; }
    static long[] arrlong() throws IOException { String line = nextLineNonEmpty(); String[] str = line.trim().split("\s+"); int n = str.length; long[] x = new long[n]; for (int i = 0; i < n; i++) x[i] = Long.parseLong(str[i]); return x; }
    static double[] arrdouble() throws IOException { String line = nextLineNonEmpty(); String[] str = line.trim().split("\s+");  int n = str.length; double[] x = new double[n]; for (int i = 0; i < n; i++) x[i] = Double.parseDouble(str[i]); return x; }
    ...
    static char[][] matchar(int n, int m) throws IOException { char[][] mat = new char[n][]; for (int i = 0; i < n; i++) mat[i] = nextLineNonEmpty().toCharArray(); return mat; }
    ...
    // ------------------ Fast Tokenizer helpers ------------------
    static String next() throws IOException { while (st == null || !st.hasMoreTokens()) { String line = br.readLine(); if (line == null) return null; st = new StringTokenizer(line); } return st.nextToken(); }
    static int nextInt() throws IOException { String s = next(); if (s == null) throw new EOFException(); return Integer.parseInt(s); }
    static long nextLong() throws IOException { String s = next(); if (s == null) throw new EOFException(); return Long.parseLong(s); }
    static String nextLineNonEmpty() throws IOException { String line = br.readLine(); while (line != null && line.trim().isEmpty()) line = br.readLine(); if (line == null) return ""; return line; }
}
