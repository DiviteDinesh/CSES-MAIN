...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    static void solve() throws IOException{
        ...
        pq.offer(new long[]{0, 0, 1}); // wt u power
        
        while(!pq.isEmpty()){
            long[] temp = pq.poll();
            long curr = temp[0];
            int u = (int)temp[1];
            int power = (int)temp[2];
            if(dist[u][power] < curr) continue;

            for(int[] edge : g[u]){
                int v = edge[1], wt = edge[2];
                if(dist[u][power] + wt < dist[v][power]){
                    dist[v][power] = dist[u][power] + wt;
                    pq.offer(new long[]{dist[v][power], v, power});
                }
                if(power == 1){
                    wt >>= 1;
                    if(curr + wt < dist[v][0]){
